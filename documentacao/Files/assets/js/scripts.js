...
$(document).ready(function() {
  ...
  function qp_required_misc() {
    ...
    $("body").on("click", function(e) {
      if (
        !$(".dropdown-no-close").is(e.target) &&
        $(".dropdown-no-close").has(e.target).length === 0 &&
    ...
    if ($(sidebarNav).length) {
      ...
      $(".sidebar > .mCustomScrollBox").before(
        '<button class="hamburger hamburger--slider" type="button" data-target=".sidebar" aria-controls="sidebar" aria-expanded="false" aria-label="Toggle Sidebar"><span class="hamburger-box"><span class="hamburger-inner"></span></span></button>'
      );
    ...
    if ($(signInRightColumn).length) {
      // Background Image
      if (
        typeof $(signInRightColumn).data("qp-bg-image") !== "undefined" &&
        $(signInRightColumn).data("qp-bg-image") != ""
      ) {
        var backgroundImage = $(signInRightColumn).data("qp-bg-image");

        $(signInRightColumn).css({
          "background-image": "url(assets/img/" + backgroundImage + ")"
        });
      }
    }
    ...
    var customColorControl = $(
      ".custom-color-control.custom-control.custom-radio"
    );
    if (customColorControl.length) {
      ...
      customColorControl.each(function() {
        ...
        $(
          "<style>.custom-color-control.custom-radio.custom-color-control-" +
            count +
    ...
    if (!$("body").hasClass("no-animation")) {
      $("[data-qp-animate-type]").each(function() {
        ...
        if (
          ...
          mainElement.closest("nav").hasClass("sidebar")
        ) {
  ...
  function qp_line_chart(chartID, maxHeight) {
    if ($(chartID).length) {
      ...
      var clickedElement = $(chartID)
        .closest(".card")
        .find(".header-btn-block .data-range.dropdown .dropdown-item");
      ...
      if (triggeredEvent == "load") {
        ...
        $(chartID)
          .closest(".card")
          .find(".header-btn-block .data-range.dropdown .dropdown-item.active")
      ...
      function load_chart(range, xAxisLabels, dataSet1, dataSet2) {
        ...
        if (canvasParent.closest(".card").hasClass("card-sm")) {
          ...
          if (
            !canvasParent.closest(".card-body").next().length &&
            !canvasParent.next().length
          ) {
            canvasParent
              .closest(".card-body")
              .css({ "padding-bottom": "0", position: "relative" });
  ...
  function qp_doughnut_pie_chart(chartID, chartType, maxWidth) {
    ...
    if ($(chartID).length) {
      ...
      function load_chart(range, dataSet) {
        ...
        var canvasParent = $(chartID).closest(".card-chart");
        var color1 = qp_hexToRgbA(
          canvasParent.data("chart-color-1"),
          areaOpacity
        );
        var color2 = qp_hexToRgbA(
          canvasParent.data("chart-color-2"),
          areaOpacity
        );
        var color3 = qp_hexToRgbA(
          canvasParent.data("chart-color-3"),
          areaOpacity
        );
        var color4 = qp_hexToRgbA(
          canvasParent.data("chart-color-4"),
          areaOpacity
        );
        var color5 = qp_hexToRgbA(
          canvasParent.data("chart-color-5"),
          areaOpacity
        );
  ...
  function qp_dial_chart(chartID) {
    if ($(chartID).length) {
      ...
      $(chartID)
        .siblings(".chart-controls")
        .find("#update-dial-chart")
        .on("click", function(e) {
  ...
  function qp_task_list() {
    ...
    if ($(taskList).length) {
      ...
      $(taskList)
        .find(".task-item-controls .show-task")
        .on("click", function(e) {
          $(this)
            .closest(".task-list-item")
            .find(".task-item-details")
            .slideToggle(300);
          e.preventDefault();
        });
      ...
      function taskCount(addCount) {
        ...
        var tasksCompleted =
          $(taskListItem)
            .closest(".card-task-list")
            .find(".active").length + addCount;
        var tasksTotal = $(taskListItem)
          .closest(".card-task-list")
          .find(".task-list-item").length;

        $(taskListItem)
          .closest(".card")
          .find(".card-header .task-list-stats .task-list-completed")
          .text(tasksCompleted);
        $(taskListItem)
          .closest(".card")
          .find(".card-header .task-list-stats .task-list-total")
          .text(tasksTotal);

        // Update Progress Bar
        var completionPercentage = (tasksCompleted / tasksTotal) * 100;

        var progressBar = $(taskListItem)
          .closest(".card")
          .find(".card-header .progress-bar");
    ...
    if ($(placeholder).length) {
      ...
      taskPriority.on("change", function() {
        if (taskPriority.val() == 4 || taskPriority.val() == 3) {
          ...
          var optionalTask = $(
            placeholder + " .input-task-optional-checkbox.custom-checkbox"
          );
      ...
      $(".assign-task").on("click", function() {
        var taskTitle = $(this)
          .closest(".create-task-block")
          .find(".input-task-title")
          .val()
          .trim();
        var taskDetails = $(this)
          .closest(".create-task-block")
          .find(".input-task-details")
          .val()
          .replace(/\n/g, "<br \\>")
          .trim();
        if (taskDetails == "") {
        ...
        var taskUsers = $(this)
          .closest(".create-task-block")
          .find(".input-task-users")
          .val()
          .trim();
        var taskUsersObj = $(this)
          .closest(".create-task-block")
          .find(".input-task-users")
          .tagsinput("items");
        var taskOptional = $(this)
          .closest(".create-task-block")
          .find(".input-task-optional")
          .is(":checked");
        var taskPriority = $(this)
          .closest(".create-task-block")
          .find(".input-task-priority")
          .val();
        ...
        for (var n = 0; n < taskUsersObj.length; n++) {
          ...
          var tempString =
            '<a class="assigned-user"><div class="profile-picture assigned-user bg-gradient bg-primary float-right d-flex"><img class="list-thumbnail" src="' +
            tempUserImage +
            '" title="' +
            tempUserName +
            '" width="30" height="30"></div></a>';
        ...
        if (taskTitle == "") {
          ...
          var taskManagerTaskList = $(this)
            .closest(".task-manager")
            .find(".card-task-list");
          ...
          var newTaskContent =
            '<li class="task-list-item"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input"><label class="custom-control-label">' +
            priorityBadge +
            optionalBadge +
            taskTitle +
            '</label></div><div class="task-item-details">' +
            taskDetails +
            '</div><div class="task-item-controls"><a role="button" class="btn btn-info btn-sm show-task" href="#"><span class="batch-icon batch-icon-arrow-down"></span></a></div><div class="task-item-users">' +
            taskUsersContent +
            "</div></li>";

          taskManagerContainer
            .find(".card-task-list .task-list")
            .prepend(newTaskContent);
          ...
          taskManagerTaskList
            ...
            .animate({ opacity: "1" }, 300, function() {
              ...
              $(this)
                .find(".task-item-users")
                .attr("data-assigned-users", taskUsers);
      ...
      $("#delete-task").on("click", function(e) {
        ...
        $(this)
          .closest(".card")
          .find(
            ".card-task-list .task-list .task-list-item .custom-checkbox.active"
          )
          .closest(".task-list-item")
          .velocity("slideUp", {
            easing: "easeOutCubic",
      ...
      function taskCount() {
        ...
        taskManagerTaskList
          ...
          .find(".task-list-stats .task-list-total")
          .text(tasksTotal);
      }
    }
  }
  ...
  function qp_calendar() {
    ...
    if ($(calendarContainer).length) {
      ...
      $(calendarContainer)
        .closest(".card-body")
        .find(
          ".calendar-controls .create-event .dropdown-menu .legend-block-item .legend-block-color-box, .calendar-controls .available-events .fc-event .legend-block-item .legend-block-color-box"
        )
        .each(function() {
          // Set variables
      ...
      $("#add-available-event").on("click", function() {
        // Set variables
        var eventColorActive = $(this)
          .siblings(".dropdown-menu")
          .find(".legend-block-item.active .legend-block-color-box")
          .data("event-color");
        var eventName = $(this)
          .parent()
          .siblings("#input-new-event")
          .val()
          .trim();
        $(this)
          .parent()
        ...
        if (eventName != "") {
          var newEventContent =
            "<div class='fc-event' style='opacity:0;'><div class='legend-block-item'><div class='legend-block-color'><div class='legend-block-color-box highlight-color-" +
            eventColorActive +
            "' data-event-color='" +
            eventColorActive +
            "'><i class='batch-icon batch-icon-droplet'></i></div></div><div class='legend-block-text'>" +
            eventName +
            "</div></div></div>";

          $(this)
            .closest(".calendar-controls")
            .find(".available-events .event-list")
            .prepend(newEventContent);

          $(this)
            .closest(".calendar-controls")
            .find(".available-events .fc-event")
            .first()
            .delay(200)
          ...
          $(this)
            .closest(".card-body")
            .find(".calendar-controls .fc-event")
            .each(function() {
              var thisEventColor = $(this)
                .find(".legend-block-color-box")
                .data("event-color");
      ...
      function getActiveColor() {
        var eventColorActive = $(calendarContainer)
          .closest(".card-body")
          .find(
            ".calendar-controls .create-event .dropdown-menu .legend-block-item.active .legend-block-color-box"
          )
          .data("event-color");

        // Set data-event-color. Then create the highlight class
        var theButton = $(calendarContainer)
          .closest(".card-body")
          .find(".calendar-controls .create-event .dropdown-toggle");
        ...
        var listItem = $(calendarContainer)
          .closest(".card-body")
          .find(
            ".calendar-controls .create-event .dropdown-menu .legend-block-item"
          );

        listItem.on("click", function() {
          var newEventColor = $(this)
            .find(".legend-block-color-box")
            .data("event-color");
          ...
          colorIndicator
            .removeAttr("class")
            .addClass(
              "legend-block-color-box highlight-color-" + newEventColor
            );
      ...
      $(calendarContainer).fullCalendar({
        ...
        eventAfterAllRender: function() {
          $(calendarContainer)
            .find(".glyphicon.glyphicon-chevron-right")
            .removeAttr("class")
            .addClass("batch-icon batch-icon-arrow-right");
          $(calendarContainer)
            .find(".glyphicon.glyphicon-chevron-left")
            .removeAttr("class")
            .addClass("batch-icon batch-icon-arrow-left");
        }
      });
    }
  }
  ...
  function qp_mailbox_list() {
    ...
    if ($(placeholder).length) {
      ...
      $(placeholder + " .email-item-checkbox .custom-control-input").removeAttr(
        "checked"
      );
      ...
      selectAll.on("click", function() {
        if ($(this).hasClass("active")) {
          $(this)
            ...
            .removeAttr("checked");
          $(
            placeholder + " .email-item-checkbox .custom-control-input"
          ).removeAttr("checked");
          ...
          $(placeholder + " .email-item-checkbox .custom-control-input").attr(
            "checked",
            "checked"
          );
          $(this).addClass("active");
          ...
          $(placeholder + " .mailbox-control-group .btn").removeClass(
            "disabled"
          );
      ...
      $(placeholder + " .email-item-checkbox").on("click", function() {
        ...
        if (thisCheckbox.find(".custom-control-input").is(":checked")) {
          ...
          if (checkedCount < 1) {
            $(placeholder + " .mailbox-control-group .btn").addClass(
              "disabled"
            );
          }
        } else {
      ...
      $(".email-mark-read").on("click", function(e) {
        ...
        $(".mailbox-email-list tr").each(function() {
          if (
            $(this).hasClass("email-status-unread") &&
            $(this)
              .find(".email-checkbox .custom-control-input")
              .is(":checked")
          ) {
      ...
      $(".email-delete").on("click", function(e) {
        ...
        var messageDelete =
          '<div class="alert alert-success alert-dismissable" style="opacity:0;"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>' +
          messageDeleteText +
          "</div>";
        ...
        $(
          ".email-mark-read, .email-mark-important, .email-mark-junk, .email-delete"
        ).addClass("disabled");
      ...
      $(".email-mark-junk").on("click", function(e) {
        ...
        var messageJunkText =
          "<strong>Moved!</strong> Email(s) have been moved to the Junk Folder.";
      ...
      $(".mailbox-email-list tr").each(function() {
        ...
        $(this)
          .find(".email-sender, .email-subject, .email-datetime")
          .on("click", function() {
  ...
  function qp_mailbox_message_compose() {
    ...
    if (placeholder.length) {
            ...
            .bind("enterKey", function(e) {
              ...
              if (
                ...
                if (isValidEmail == true) {
                  ...
                  var dynamicID =
                    placeholderObj.siblings(".bootstrap-tagsinput").find(".tag")
                      .length + 1000;

                  placeholderObj.tagsinput("add", {
                    value: dynamicID,
                    text: newEmail,
                    email: newEmail
                  });
      ...
      $("#email-recepient-main").tagsinput("add", {
        value: 5,
        text: "Teal'c Jafar",
        email: "tealc@example.com"
      });
      $("#email-recepient-cc").tagsinput("add", {
        value: 2,
      ...
      $("#email-recepient-cc").tagsinput("add", {
        value: 4,
        text: "Danielle Jackson",
        email: "danielle@example.com"
      });
      $("#email-recepient-bcc").tagsinput("add", {
        value: 6,
        text: "George Hammond",
      ...
      $("#send-email").on("click", function(e) {
        ...
        if (emailsMainObj == "") {
          ...
          var errorMessage =
            '<div class="alert alert-danger alert-dismissable text-left" style="display:none;"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><strong>ERROR (DEMO ONLY)</strong><br /><br />The "To" field cannot be empty</div>';
          ...
          var errorMessage =
            '<div class="alert alert-danger alert-dismissable text-left" style="display:none;"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><strong>ERROR (DEMO ONLY)</strong><br /><br />The "Subject" field cannot be empty</div>';
          ...
          var emailsMainRecepient =
            "<strong>To:</strong> " + emailsMain.toString() + "<br />";

          for (var i = 0; i < emailsCcObj.length; i++) {
            emailsCc.push(emailsCcObj[i].email);
          }
          var emailsCcText =
            "<strong>Cc:</strong> " + emailsCc.toString() + "<br />";

          for (var i = 0; i < emailsBccObj.length; i++) {
            emailsBcc.push(emailsBccObj[i].email);
          }
          var emailsBccText =
            "<strong>Bcc:</strong> " + emailsBcc.toString() + "<br />";

          // Main Message Content
          var emailContent = $("#email-body").html();

          var successMessage =
            '<div class="alert alert-success alert-dismissable text-left" style="display:none;"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><strong>MESSAGE SENT (DEMO ONLY)</strong><br /><br />' +
            emailsMainRecepient +
            emailsCcText +
            emailsBccText +
            "</div>";
      ...
      $(thisForm).bootstrapWizard({
        onNext: function(tab, navigation, index) {
          ...
          if (index == 1) {
            if (
              qp_form_validation(
                "#wizard-stage-1 .wizard-stage-1-username",
                "alphanumeric"
              ) !== true
            ) {
              return false;
            ...
            if (
              qp_form_validation(
                "#wizard-stage-1 .wizard-stage-1-email",
                "email"
              ) !== true
            ) {
              return false;
            ...
            if (
              qp_form_validation(
                "#wizard-stage-1 .wizard-stage-1-password",
                "password"
              ) !== true
            ) {
              return false;
          ...
          if (index == 2) {
            ...
            $(".form-wizard-review-block").append(
              "<p><strong>Username:</strong> " +
                $(".wizard-stage-1-username").val() +
                "</p>"
            );
            $(".form-wizard-review-block").append(
              "<p><strong>Email:</strong> " +
                $(".wizard-stage-1-email").val() +
                "</p>"
            );
            $(".form-wizard-review-block").append(
            ...
            $(".form-wizard-review-block").append(
              "<p><strong>Telephone:</strong> " +
                $(".wizard-stage-2-optional-1").val() +
                "</p>"
            );
            $(".form-wizard-review-block").append(
              "<p><strong>Your Address:</strong> " +
                $(".wizard-stage-2-optional-2").val() +
                "</p>"
            );
            $(".form-wizard-review-block").append(
              "<p><strong>Write something about yourself:</strong> " +
                $(".wizard-stage-2-optional-3").val() +
                "</p>"
            );
  ...
  function qp_form_validation(fieldID, type, required, lengthMin, lengthMax) {
    ...
    switch (type) {
      case "email":
        ...
        if (qp_form_validate_email(value) === true) {
          ...
          var message =
            '<div class="form-control-feedback text-danger">Please enter a valid email.</div>';
      ...
      case "numeric":
        if (required && value.trim() == "") {
          var message =
            '<div class="form-control-feedback text-danger">This field is required.</div>';
        ...
        if (qp_form_validate_numeric(value) === true) {
          ...
          var message =
            '<div class="form-control-feedback text-danger">Please enter numbers only.</div>';
      ...
      case "alphanumeric":
        ...
        if (qp_form_validate_alphanumeric(value) == true) {
          ...
          var message =
            '<div class="form-control-feedback text-danger">This field allows only letters numbers, commas and fullstops but NO spaces.</div>';
      ...
      case "alphabet":
        ...
        if (qp_form_validate_alphabet(value) == true) {
          ...
          var message =
            '<div class="form-control-feedback text-danger">This field allows letters only.</div>';
    ...
    function qp_form_validate_password(password, lengthMin, lengthMax) {
      var passwordLengthStatus = qp_form_validate_stringlength(
        password,
        lengthMin,
        lengthMax
      );
      if (passwordLengthStatus != true) {
        ...
        if (passwordStatus == true) {
          ...
          var message =
            "Password field can only contain letters, numbers, commas and fullstops but NO spaces.";
          return message;
        }
      }
    }
    ...
    function qp_form_validate_stringlength(str, lengthMin, lengthMax) {
      ...
      if (n < lengthMin) {
        var message =
          "This field must be at least " + lengthMin + " characters long";
        return message;
      } else if (n > lengthMax) {
        var message =
          "This field must not be more than " + lengthMax + " characters long";
        return message;
      } else {
        return true;
      }
