...
(function() {
  ...
  function x(a, b) {
    ...
    for (c = 0; c < d.length; c++) f.add(d[c]);
  }
  var g,
    k,
    t,
  ...
  CKEDITOR.cleanWord = function(a, b) {
    ...
    var d = CKEDITOR.htmlParser.fragment.fromHtml(a);
    q = new CKEDITOR.htmlParser.filter({
      root: function(e) {
        e.filterChildren(q);
        CKEDITOR.plugins.pastefromword.lists.cleanup(g.createLists(e));
      },
      elementNames: [
        [/^\?xml:namespace$/, ""],
        [/^v:shapetype/, ""],
      ...
      elements: {
        a: function(e) {
          ...
          if (e.attributes.href && e.attributes.href.match(/#.+$/)) {
            var a = e.attributes.href.match(/#(.+)$/)[1];
            w[a] = e;
          }
          e.attributes.name &&
            w[e.attributes.name] &&
            ((e = w[e.attributes.name]),
            (e.attributes.href = e.attributes.href.replace(/.*#(.*)$/, "#$1")));
        },
        div: function(e) {
        ...
        img: function(e) {
          if (e.parent && e.parent.attributes) {
            var a = e.parent.attributes;
            (a = a.style || a.STYLE) &&
              a.match(/mso\-list:\s?Ignore/) &&
              (e.attributes["cke-ignored"] = !0);
          }
          k.mapStyles(e, {
            width: function(a) {
          ...
          e.attributes.src &&
            e.attributes.src.match(/^file:\/\//) &&
            e.attributes.alt &&
            e.attributes.alt.match(/^https?:\/\//) &&
            (e.attributes.src = e.attributes.alt);
        },
        p: function(a) {
          a.filterChildren(q);
          if (
          ...
          if (g.thisIsAListItem(b, a))
            ...
            var c = a.getAscendant(function(a) {
              ...
              d = n.parseCssText(a.attributes.style);
            c &&
        ...
        font: function(a) {
          ...
          CKEDITOR.tools.arrayCompare(CKEDITOR.tools.objectKeys(a.attributes), [
            "class",
            "style"
          ])
            ? k.createStyleStack(a, q, b)
        ...
        table: function(a) {
          ...
          if (
            ...
            "div" === b.name &&
            b.attributes.align &&
            1 === n.objectKeys(b.attributes).length &&
            1 === b.children.length
          ) {
        ...
        td: function(a) {
          var c = a.getAscendant("table"),
            ...
            m = c.background || c.BACKGROUND;
          m && k.setStyle(a, "background", m, !0);
          (c = c["background-color"] || c["BACKGROUND-COLOR"]) &&
            k.setStyle(a, "background-color", c, !0);
          var c = n.parseCssText(a.attributes.style),
            p;
          for (p in c) (m = c[p]), delete c[p], (c[p.toLowerCase()] = m);
  ...
  CKEDITOR.plugins.pastefromword.styles = {
    ...
    normalizedStyles: function(a, b) {
      var c = "background-color:transparent border-image:none color:windowtext direction:ltr mso- text-indent visibility:visible div:border:none".split(
        ...
        h = n.parseCssText(a.attributes.style);
      "cke:li" == a.name &&
        h["TEXT-INDENT"] &&
        h.MARGIN &&
      ...
      for (var l = n.objectKeys(h), r = 0; r < l.length; r++) {
        var m = l[r].toLowerCase(),
          p = h[l[r]],
          k = CKEDITOR.tools.indexOf;
        ((e && -1 !== k(d, m.toLowerCase())) ||
          f(null, m, p) ||
          f(null, m.replace(/\-.*$/, "-")) ||
          f(null, m) ||
          f(a.name, m, p) ||
    ...
    createStyleStack: function(a, b, c, d) {
      ...
      k.sortStyles(a);
      b = n.parseCssText(k.normalizedStyles(a, c));
      c = a;
      var e = "span" === a.name,
        h;
      for (h in b)
    ...
    sortStyles: function(a) {
      ...
      f.sort(function(a, c) {
        var d = n.indexOf(b, a.toLowerCase()),
          f = n.indexOf(b, c.toLowerCase());
        return d - f;
      });
      d = [].concat(f, e);
      f = {};
      for (h = 0; h < d.length; h++) f[d[h]] = c[d[h]];
      a.attributes.style = CKEDITOR.tools.writeCssText(f);
    },
        ...
        if (
          ...
          for (var h = !1, l = 0; l < a.children.length; l++) {
            ...
            if (g.type === CKEDITOR.NODE_TEXT && c) {
              var m = new CKEDITOR.htmlParser.element("span");
              m.setHtml(g.value);
              g.replaceWith(m);
              g = m;
            }
            g.type === CKEDITOR.NODE_ELEMENT &&
              ((h = !0), k.setStyle(g, e, f[e]));
          }
          h && delete f[e];
        }
      a.attributes.style = CKEDITOR.tools.writeCssText(f);
    ...
    inliner: {
      ...
      parse: function(a) {
        ...
        var h;
        if (e)
          for (e = e.cssRules, h = 0; h < e.length; h++)
            e[h].type === window.CSSRule.STYLE_RULE &&
              a.push({
                selector: e[h].selectorText,
                styles: f(c(e[h].cssText))
              });
        return a;
      },
      filter: function(a) {
  ...
  CKEDITOR.plugins.pastefromword.lists = {
    ...
    convertToFakeListItem: function(a, b) {
      ...
      if (b.attributes.style) {
        var d = n.parseCssText(b.attributes.style);
        d["margin-left"] &&
    ...
    removeSymbolText: function(a) {
      ...
      a.forEach(function(d) {
        !b &&
          d.value.match(c.replace(")", "\\)").replace("(", "")) &&
    ...
    setListSymbol: function(a, b, c) {
      c = c || 1;
      var d = n.parseCssText(a.attributes.style);
      if ("ol" == a.name) {
      ...
      for (a = 0; a < e.length; a++) {
        ...
        for (d = 0; d < h.length; d++) {
          ...
          for (c = b.attributes["cke-list-level"]; c > l.length; ) {
            ...
            0 < n.length
              ? n[n.length - 1].add(p)
              : ((n = new CKEDITOR.htmlParser.element("li", {
            ...
            m.push(p);
            k = p;
            c == l.length && g.setListSymbol(p, b.attributes["cke-symbol"], c);
          }
          for (; c < l.length; )
            l.pop(),
              (k = l[l.length - 1]),
        ...
        for (d = 0; d < m.length; d++) g.setListStart(m[d]);
    ...
    calculateValue: function(a) {
      ...
      var c = null,
        ...
        f,
        e;
      for (e = a; 0 <= e && null === c; e--)
        (f = b.children[e]),
          f.attributes &&
            void 0 !== f.attributes.value &&
            ((d = e), (c = parseInt(f.attributes.value, 10)));
      null === c &&
        ((c =
    ...
    dissolveList: function(a) {
      ...
      e.forEach(m, function(a) {
        var g = a.attributes.type,
          ...
          m = c(f, a) + 1;
        g || (g = n.parseCssText(a.attributes.style)["list-style-type"]);
        e.forEach(e.filter(a.children, d("li")), function(c, d) {
          var e;
          switch (g) {
            case "disc":
              e = "·";
            ...
            case "a":
            case "lower-alpha":
              e = String.fromCharCode(97 + h - 1 + d) + ".";
              break;
            case "A":
            case "upper-alpha":
              e = String.fromCharCode(65 + h - 1 + d) + ".";
              break;
            case "i":
            case "lower-roman":
              e = b(h + d) + ".";
      ...
      for (r = l.length - 1; 0 <= r; r--) l[r].insertAfter(a);
      for (r = m.length - 1; 0 <= r; r--) delete m[r].name;
    },
    groupLists: function(a) {
      ...
      for (b = 1; b < a.length; b++) {
        ...
        c.attributes["cke-indentation"] =
          c.attributes["cke-indentation"] || g.getElementIndentation(c);
        c.previous !== e && (g.chopDiscontinuousLists(f, d), d.push((f = [])));
        f.push(c);
      }
      g.chopDiscontinuousLists(f, d);
      return d;
    },
    chopDiscontinuousLists: function(a, b) {
      for (var c = {}, d = [[]], f, e = 0; e < a.length; e++) {
        var h = c[a[e].attributes["cke-list-level"]],
          ...
          m;
        h
          ? ((m = h.type.match(/alpha/) && 7 == h.index ? "alpha" : m),
            (m =
              "o" == a[e].attributes["cke-symbol"] && 14 == h.index
                ? "alpha"
                : m),
            (k = g.getSymbolInfo(a[e].attributes["cke-symbol"], m)),
            (l = this.getListItemInfo(a[e])),
            (h.type != k.type ||
              (f && l.id != f.id && !this.isAListContinuation(a[e]))) &&
              d.push([]))
          : (k = g.getSymbolInfo(a[e].attributes["cke-symbol"]));
        for (
          f = parseInt(a[e].attributes["cke-list-level"], 10) + 1;
          20 > f;
          f++
        )
          c[f] && delete c[f];
        c[a[e].attributes["cke-list-level"]] = k;
        d[d.length - 1].push(a[e]);
        f = l;
      }
    ...
    getElementIndentation: function(a) {
      a = n.parseCssText(a.attributes.style);
    ...
    getListItemInfo: function(a) {
      ...
      var b = n.parseCssText(a.attributes.style)["mso-list"],
        c = { id: "0", level: "1" };
      b &&
        ((b += " "),
        ...
        (b.attributes.style &&
          ...
          !!b.find(function(a) {
            if (
              ...
              return !0;
            var d = n.parseCssText(a.attributes && a.attributes.style, !0);
            if (!d) return !1;
            var f = d["font-family"] || "";
            return (
    ...
    assignListLevels: function(a, b) {
      if (!b.attributes || void 0 === b.attributes["cke-list-level"]) {
        ...
        var k = h(c, function(a, b) {
          ...
          n = this.guessIndentationStep(
            e.filter(c, function(a) {
              return 0 !== a;
            })
          ),
          f = h(c, function(a) {
