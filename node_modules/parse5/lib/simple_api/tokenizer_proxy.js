...
TokenizerProxy.prototype._handleStartTagToken = function(token) {
  ...
  if (tn === $.SVG) this._enterNamespace(NS.SVG);
    ...
    if (this.inForeignContent) {
      if (ForeignContent.causesExit(token)) this._leaveCurrentNamespace();
      else if (
        ForeignContent.isMathMLTextIntegrationPoint(
          tn,
          this.currentNamespace
        ) ||
        ForeignContent.isHtmlIntegrationPoint(
          tn,
          this.currentNamespace,
          token.attrs
        )
...
TokenizerProxy.prototype._handleEndTagToken = function(token) {
  ...
  if (!this.inForeignContent) {
    ...
    if (
      ForeignContent.isMathMLTextIntegrationPoint(tn, previousNs) ||
