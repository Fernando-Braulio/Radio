...
FSWatcher.prototype._emit = function(event, path, val1, val2, val3) {
  ...
  if (this.options.atomic) {
    if (event === "unlink") {
      ...
      setTimeout(
        function() {
          Object.keys(this._pendingUnlinks).forEach(
            function(path) {
  ...
  if (
    ...
    var fullPath = this.options.cwd
      ? sysPath.join(this.options.cwd, path)
      : path;
...
FSWatcher.prototype._awaitWriteFinish = function(
  path,
  threshold,
  event,
  awfEmit
    ...
    if (cwd && ignored) {
      ignored = ignored.map(function(path) {
        ...
        return upath.normalize(
          isAbsolute(path) ? path : sysPath.join(cwd, path)
        );
      });
    }
...
FSWatcher.prototype._getWatchHelpers = function(path, depth) {
  ...
  var checkGlobSymlink = function(entry) {
    ...
    if (globSymlink == null) {
      globSymlink =
        entry.fullParentDir === fullWatchPath
          ? false
          : {
  ...
  var filterPath = function(entry) {
    ...
    return (
      (!hasGlob || globFilter(resolvedPath)) &&
    ...
    if (hasGlob) {
      ...
      unmatchedGlob = !dirParts.some(function(parts) {
        return parts.every(function(part, i) {
          ...
          return (
            globstar || !entryParts[0][i] || anymatch(part, entryParts[0][i])
          );
        });
      });
    }
...
FSWatcher.prototype._hasReadPermissions = function(stats) {
  return Boolean(
    4 & parseInt(((stats && stats.mode) & 0x1ff).toString(8)[0], 10)
  );
};
...
FSWatcher.prototype.getWatched = function() {
  var watchList = {};
  Object.keys(this._watched).forEach(
    function(dir) {
      var key = this.options.cwd
