  ...
  setTimeout(function() {
    while (i--)
      child.uptime(function(err, uptime) {
        t.ok(
          uptime > 10,
          "child has been up before the request (" + uptime + "ms)"
        );
      });
    ...
    child(0, function(err, pid) {
      ...
      if (pids.length == count) {
        t.equal(
          count / 2,
          uniq(pids).length,
          "one process for each call (by pid)"
        );
        workerFarm.end(child, function() {
          t.ok(true, "workerFarm ended");
        });
    ...
    while (i--) {
      child(defer, function() {
        if (++cbc == count) {
          ...
          t.ok(
            time > defer * count && time < defer * count * 1.3,
            "processed tasks sequentially (" + time + ")"
          );
          workerFarm.end(child, function() {
            t.ok(true, "workerFarm ended");
          });
...
tape('custom arguments can be passed to "fork"', function(t) {
  ...
  let cwd = fs.realpathSync(os.tmpdir()),
    ...
    child = workerFarm(
      { maxConcurrentWorkers: 1, maxRetries: 5, workerOptions: workerOptions },
      childPath,
      ["args"]
    );
  ...
  child.args(function(err, result) {
    t.equal(
      result.execArgv[0],
      "--expose-gc",
      "flags passed (overridden default)"
    );
