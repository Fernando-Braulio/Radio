  ...
  test(mode, function(t) {
    ...
    function runOnce(i) {
      t.test("run: " + i, function(t) {
        ...
        var ourCipherText = Buffer.concat([
          ourEncrypt.update(text),
          ourEncrypt.final()
        ]);
        var nodeCipherText = Buffer.concat([
          nodeEncrypt.update(text),
          nodeEncrypt.final()
        ]);
        ...
        var plainText = Buffer.concat([
          ourDecrypt.update(ourCipherText),
          ourDecrypt.final()
        ]);
      ...
      t.test("run text: " + i, function(t) {
        ...
        var key = crypto
          .randomBytes(32)
          .toString("base64")
          .slice(0, modes[mode].key);
        var iv = crypto
          .randomBytes(32)
          .toString("base64")
          .slice(0, modes[mode].iv);
        var text = crypto.randomBytes(200);
        var ourEncrypt;
        try {
          ourEncrypt = new DES({
