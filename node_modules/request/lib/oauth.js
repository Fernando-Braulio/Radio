...
OAuth.prototype.buildParams = function(
  ...
  var consumer_secret_or_private_key =
    oa.oauth_consumer_secret || oa.oauth_private_key; // eslint-disable-line camelcase
...
OAuth.prototype.buildBodyHash = function(_oauth, body) {
  if (
    ["HMAC-SHA1", "RSA-SHA1"].indexOf(_oauth.signature_method || "HMAC-SHA1") <
    0
  ) {
    this.request.emit(
      "error",
  ...
  switch (transport) {
    case "header":
      self.request.setHeader(
        "Authorization",
        "OAuth " + self.concatParams(oa, ",", '"')
      );
      break;

    case "query":
      var href = (self.request.uri.href +=
        (query ? "&" : "?") + self.concatParams(oa, "&"));
