...
class Compilation extends Tapable {
        ...
        return callback(
          new Error(
            `No module factory available for dependency type: ${
              dependencies[i][0].constructor.name
            }`
          )
        );
      }
  ...
  processDependenciesBlocksForChunks(inputChunks) {
    ...
    const iteratorBlock = b => {
      ...
      if (c === undefined) {
        ...
        if (c && c.isInitial()) {
          ...
          this.warnings.push(
            new AsyncDependencyToInitialChunkWarning(
              b.chunkName,
              b.module,
              b.loc
            )
    ...
    const filterFn = dep => {
      ...
      if (areModulesAvailable(depChunk, newAvailableModules)) return false; // break all modules are already available
      return true;
    };
    ...
    for (const chunk of allCreatedChunks) {
      if (chunk.parents.length === 0) chunk.remove("unconnected");
    }
  }
