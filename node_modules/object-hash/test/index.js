...
describe("hash", function() {
  ...
  if (typeof Buffer !== "undefined") {
    it("can return buffers", function() {
      assert.ok(
        Buffer.isBuffer(
          hash({ foo: "bar", bar: "baz" }, { encoding: "buffer" })
        ),
        "hash object"
      );
    });
  ...
  it("sugar methods should be equivalent", function() {
    ...
    assert.equal(
      hash.keysMD5(obj),
      hash(obj, { algorithm: "md5", excludeValues: true }),
      "keys md5"
    );
  });
  ...
  it("recursive objects don't blow up stack", function() {
    ...
    assert.doesNotThrow(
      ...
      "Should not throw an stack size exceeded exception"
    );
  });
  ...
  it("recursive arrays don't blow up stack with unorderedArrays", function() {
    ...
    assert.doesNotThrow(
      function() {
        hash(hash1, { unorderedArrays: true });
      },
      /Maximum call stack size exceeded/,
      "Should not throw an stack size exceeded exception"
    );
  ...
  it("recursive handling tracks identity", function() {
    ...
    assert.notEqual(
      hash(hash1),
      hash(hash2),
      "order of recursive objects should matter"
    );
  });
  ...
  it("object types are hashed", function() {
    ...
    assert.notEqual(
      hash1,
      hash2,
      "arrays and objects should not produce identical hashes"
    );
  });
  ...
  it("utf8 strings are hashed correctly", function() {
    ...
    assert.notEqual(
      hash1,
      hash2,
      "different strings with similar utf8 encodings should produce different hashes"
    );
  });
  ...
  it("various hashes in crypto.getHashes() should be supported", function() {
    ...
    if (crypto.getHashes) {
      ...
      hashes = crypto
        .getHashes()
        .filter(RegExp.prototype.test.bind(/^(md|sha)/i));
    }
  ...
  it("null and 'Null' string produce different hashes", function() {
    ...
    assert.notEqual(
      hash1,
      hash2,
      "null and 'Null' should not produce identical hashes"
    );
  });
  ...
  it("Distinguish functions based on prototype properties", function() {
    ...
    assert.notEqual(
      a,
      b,
      "adding a property to the prototype changes the hash"
    );
    assert.notEqual(
      b,
      c,
      "adding another property to the prototype changes the hash"
    );
    assert.notEqual(
      c,
      d,
      "changing a property in the prototype changes the hash"
    );
  });
  ...
  it("Distinguish objects based on their type", function() {
    ...
    assert.notEqual(
      hash(Foo),
      hash(Bar),
      "Functions with different names should produce a different Hash."
    );
    assert.notEqual(
      hash(f),
      hash(b),
      "Objects with different constructor should have a different Hash."
    );
  });
  ...
  it("respectType = false", function() {
    ...
    assert.equal(
      hash(f, opt),
      hash(b, opt),
      "Hashing should disregard the different constructor"
    );
    ...
    assert.equal(
      ha,
      hb,
      "Hashing should disregard changes in the function's prototype"
    );
  });
  ...
  it("unorderedArrays = false", function() {
    ...
    assert.notEqual(
      ha,
      hb,
      "Hashing should respect the order of array entries"
    );
  });
  ...
  it("unorderedArrays = true", function() {
    ...
    assert.equal(
      ha,
      hb,
      "Hashing should not respect the order of array entries"
    );
  ...
  if (typeof Set !== "undefined") {
    it("unorderedSets = false", function() {
      ...
      assert.notEqual(
        ha,
        hb,
        "Hashing should respect the order of Set entries"
      );
    });
    ...
    it("unorderedSets = true", function() {
      ...
      assert.equal(
        ha,
        hb,
        "Hashing should not respect the order of Set entries"
      );
    });
  }
});
