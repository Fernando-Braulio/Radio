...
var getArraySubclassWithSpeciesConstructor = function getArraySubclass(
	speciesConstructor
) {
...
var testIterator = function(t, iterator, expected) {
	...
	while (((result = iterator.next()), !result.done)) {
		...
		t.deepEqual(
			result,
			{ done: false, value: expected[resultCount] },
			"result " + resultCount
		);
		resultCount += 1;
	}
	t.equal(
		resultCount,
		expected.length,
		"expected " + expected.length + ", got " + resultCount
	);
};
...
var testEnumerableOwnNames = function(t, enumerableOwnNames) {
	...
	t.equal(
		Object.prototype.propertyIsEnumerable.call(obj, "own"),
		true,
		'has enumerable "own"'
	);
	...
	t.equal(
		has(Child.prototype, "inherited"),
		true,
		'Child.prototype has own "inherited"'
	);
	t.equal(
		Child.prototype.inherited,
		obj.inherited,
		"Child.prototype.inherited === obj.inherited"
	);
	t.equal(
		Object.prototype.propertyIsEnumerable.call(Child.prototype, "inherited"),
		true,
		'has enumerable "inherited"'
	);

	t.equal("toString" in obj, true, 'has "toString"');
	t.equal(has(obj, "toString"), false, 'has non-own "toString"');
	t.equal(
		has(Object.prototype, "toString"),
		true,
	...
	t.equal(
		Object.prototype.propertyIsEnumerable.call(Object.prototype, "toString"),
		false,
		'has non-enumerable "toString"'
	);
...
var es2015 = function ES2015(ES, ops, expectedMissing, skips) {
	...
	test("ToNumber", function(t) {
		...
		t.test("numbers", function(st) {
			...
			forEach(["foo", "0", "4a", "2.0", "Infinity", "-Infinity"], function(
				numString
			) {
				st.ok(
		...
		t.test("trimming of whitespace and non-whitespace characters", function(
			st
		) {
			var whitespace =
				" \t\x0b\f\xa0\ufeff\n\r\u2028\u2029\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000";
	...
	test("IsRegExp", function(t) {
		...
		t.test("Symbol.match", { skip: !v.hasSymbols || !Symbol.match }, function(
			st
		) {
			var obj = {};
	...
	test("IsInteger", function(t) {
		...
		var notInts = v.nonNumbers.concat(v.nonIntegerNumbers, v.infinities, [
			NaN,
			[],
			new Date()
		]);
	...
	test("SpeciesConstructor", function(t) {
		...
		t["throws"](
			function() {
				return ES.SpeciesConstructor({ constructor: null }, defaultConstructor);
			},
			TypeError,
	...
	test(
		...
		function(t) {
			...
			t.equal(
				ES.IsPropertyDescriptor(v.accessorDescriptor()),
				true,
				"accessor descriptor is a Property Descriptor"
			);
			t.equal(
				ES.IsPropertyDescriptor(v.mutatorDescriptor()),
				true,
				"mutator descriptor is a Property Descriptor"
			);
			t.equal(
				ES.IsPropertyDescriptor(v.dataDescriptor()),
				true,
				"data descriptor is a Property Descriptor"
			);
			t.equal(
				ES.IsPropertyDescriptor(v.genericDescriptor()),
				true,
	...
	test("IsAccessorDescriptor", function(t) {
		...
		t.equal(
			ES.IsAccessorDescriptor(v.accessorDescriptor()),
			true,
			"accessor descriptor is an Accessor Descriptor"
		);
		t.equal(
			ES.IsAccessorDescriptor(v.mutatorDescriptor()),
			true,
			"mutator descriptor is an Accessor Descriptor"
		);
		t.equal(
			ES.IsAccessorDescriptor(v.dataDescriptor()),
			false,
			"data descriptor is not an Accessor Descriptor"
		);
		t.equal(
			ES.IsAccessorDescriptor(v.genericDescriptor()),
			false,
	...
	test("IsDataDescriptor", function(t) {
		...
		t.equal(
			ES.IsDataDescriptor(v.accessorDescriptor()),
			false,
			"accessor descriptor is not a Data Descriptor"
		);
		t.equal(
			ES.IsDataDescriptor(v.mutatorDescriptor()),
			false,
			"mutator descriptor is not a Data Descriptor"
		);
		t.equal(
			ES.IsDataDescriptor(v.dataDescriptor()),
			true,
			"data descriptor is a Data Descriptor"
		);
		t.equal(
			ES.IsDataDescriptor(v.genericDescriptor()),
			false,
	...
	test("IsGenericDescriptor", function(t) {
		...
		t.equal(
			ES.IsGenericDescriptor(v.accessorDescriptor()),
			false,
			"accessor descriptor is not a generic Descriptor"
		);
		t.equal(
			ES.IsGenericDescriptor(v.mutatorDescriptor()),
			false,
			"mutator descriptor is not a generic Descriptor"
		);
		t.equal(
			ES.IsGenericDescriptor(v.dataDescriptor()),
			false,
			"data descriptor is not a generic Descriptor"
		);

		t.equal(
			ES.IsGenericDescriptor(v.genericDescriptor()),
			true,
	...
	test("RegExpExec", function(t) {
		...
		t.test("actual regex that should match against a string", function(st) {
			...
			st.deepEqual(
				match1,
				assign(["a"], groups({ index: 0, input: S })),
				"match object 1 is as expected"
			);
			st.deepEqual(
				match2,
				assign(["a"], groups({ index: 1, input: S })),
	...
	test("ArraySpeciesCreate", function(t) {
		...
		t.test("works with species construtor", { skip: !hasSpecies }, function(
			st
		) {
			var sentinel = {};
	...
	test("CreateDataProperty", function(t) {
		...
		forEach(v.propertyKeys, function(propertyKey) {
			...
			if (typeof Object.defineProperty === "function") {
				var nonWritable = Object.defineProperty({}, propertyKey, {
					configurable: true,
					writable: false
				});

				var nonWritableStatus = ES.CreateDataProperty(
					nonWritable,
					propertyKey,
					sentinel
				);
				...
				var nonConfigurable = Object.defineProperty({}, propertyKey, {
					configurable: false,
					writable: true
				});

				var nonConfigurableStatus = ES.CreateDataProperty(
					nonConfigurable,
					propertyKey,
					sentinel
				);
	...
	test("ObjectCreate", function(t) {
		...
		t.test("proto arg", function(st) {
			...
			st.equal(
				child.foo,
				Parent.prototype.foo,
				"child inherits properties from Parent.prototype"
			);
		...
		t.test("null proto", { skip: !Object.create }, function(st) {
			...
			st.equal(
				"toString" in ES.ObjectCreate(null),
				false,
				"makes a null object"
			);
		...
		t.test(
			"null proto when no native Object.create",
			{ skip: Object.create },
			function(st) {
				st["throws"](
					function() {
	...
	test("AdvanceStringIndex", function(t) {
		...
		t.test("non-unicode mode", function(st) {
			for (var i = 0; i < str.length + 2; i += 1) {
				st.equal(
					ES.AdvanceStringIndex(str, i, false),
					i + 1,
					i + " advances to " + (i + 1)
				);
			}
	...
	test("CreateMethodProperty", function(t) {
		...
		t.test("defines correctly", function(st) {
			...
			st.equal(
				ES.CreateMethodProperty(obj, key, value),
				true,
				"defines property successfully"
			);
			st.test(
				"property descriptor",
				{ skip: !Object.getOwnPropertyDescriptor },
				function(s2t) {
					s2t.deepEqual(
		...
		t.test(
			"fails as expected on a frozen object",
			{ skip: !Object.freeze },
			function(st) {
				var obj = Object.freeze({ foo: "bar" });
				st["throws"](
					function() {
						ES.CreateMethodProperty(obj, "foo", { value: "baz" });
					},
					TypeError,
		...
		t.test(
			"fails as expected on a function with a nonconfigurable name",
			{ skip: !hasNonConfigurableFunctionName },
			function(st) {
				st["throws"](
					function() {
						ES.CreateMethodProperty(function() {}, "name", { value: "baz" });
					},
					TypeError,
	...
	test("DefinePropertyOrThrow", function(t) {
		...
		t.test("defines correctly", function(st) {
			...
			st.equal(
				ES.DefinePropertyOrThrow(obj, key, descriptor),
				true,
				"defines property successfully"
			);
			st.test(
	...
	test("DeletePropertyOrThrow", function(t) {
		...
		t.test("defines correctly", function(st) {
			...
			st.equal(
				ES.DeletePropertyOrThrow(obj, key),
				true,
	...
	test("thisNumberValue", function(t) {
		...
		forEach(v.numbers, function(number) {
			t.equal(
				ES.thisNumberValue(number),
				number,
				debug(number) + " is its own thisNumberValue"
			);
			var obj = Object(number);
			t.equal(
				ES.thisNumberValue(obj),
				number,
				debug(obj) + " is the boxed thisNumberValue"
			);
		});
	...
	test("thisBooleanValue", function(t) {
		...
		forEach(v.booleans, function(boolean) {
			t.equal(
				ES.thisBooleanValue(boolean),
				boolean,
				debug(boolean) + " is its own thisBooleanValue"
			);
			var obj = Object(boolean);
			t.equal(
				ES.thisBooleanValue(obj),
				boolean,
				debug(obj) + " is the boxed thisBooleanValue"
			);
		});
	...
	test("thisStringValue", function(t) {
		...
		forEach(v.strings, function(string) {
			t.equal(
				ES.thisStringValue(string),
				string,
				debug(string) + " is its own thisStringValue"
			);
			var obj = Object(string);
			t.equal(
				ES.thisStringValue(obj),
				string,
				debug(obj) + " is the boxed thisStringValue"
			);
		});
	...
	test("thisTimeValue", function(t) {
		...
		forEach(v.timestamps, function(timestamp) {
			...
			t.equal(
				ES.thisTimeValue(date),
				timestamp,
				debug(date) + " is its own thisTimeValue"
			);
		});
	...
	test("TestIntegrityLevel", function(t) {
		...
		t.equal(
			ES.TestIntegrityLevel({ a: 1 }, "sealed"),
			false,
			"basic object is not sealed"
		);
		t.equal(
			ES.TestIntegrityLevel({ a: 1 }, "frozen"),
			false,
			"basic object is not frozen"
		);

		t.test("preventExtensions", { skip: !Object.preventExtensions }, function(
			st
		) {
			var o = Object.preventExtensions({ a: 1 });
			st.equal(
				ES.TestIntegrityLevel(o, "sealed"),
				false,
				"nonextensible object is not sealed"
			);
			st.equal(
				ES.TestIntegrityLevel(o, "frozen"),
				false,
				"nonextensible object is not frozen"
			);

			var empty = Object.preventExtensions({});
			st.equal(
				ES.TestIntegrityLevel(empty, "sealed"),
				true,
				"empty nonextensible object is sealed"
			);
			st.equal(
				ES.TestIntegrityLevel(empty, "frozen"),
				true,
				"empty nonextensible object is frozen"
			);
			st.end();
		});
		...
		t.test("seal", { skip: !Object.seal }, function(st) {
			...
			st.equal(
				ES.TestIntegrityLevel(empty, "sealed"),
				true,
				"empty sealed object is sealed"
			);
			st.equal(
				ES.TestIntegrityLevel(empty, "frozen"),
				true,
				"empty sealed object is frozen"
			);
	...
	test("OrdinaryHasInstance", function(t) {
		forEach(v.nonFunctions, function(nonFunction) {
			t.equal(
				ES.OrdinaryHasInstance(nonFunction, {}),
				false,
		...
		forEach(v.primitives, function(primitive) {
			t.equal(
				ES.OrdinaryHasInstance(function() {}, primitive),
				false,
				debug(primitive) + " is not an object"
			);
		});
		...
		var D = function D() {};
		t.equal(
			ES.OrdinaryHasInstance(C, new C()),
			true,
			"constructor function has an instance of itself"
		);
		t.equal(
			ES.OrdinaryHasInstance(C, new D()),
			false,
			"constructor/instance mismatch is false"
		);
		t.equal(
			ES.OrdinaryHasInstance(D, new C()),
			false,
			"instance/constructor mismatch is false"
		);
		t.equal(
			ES.OrdinaryHasInstance(C, {}),
			false,
			"plain object is not an instance of a constructor"
		);
		t.equal(
			ES.OrdinaryHasInstance(Object, {}),
			true,
			"plain object is an instance of Object"
		);
	...
	test("OrdinaryHasProperty", function(t) {
		...
		t.equal(
			ES.OrdinaryHasProperty({ a: 1 }, "a"),
			true,
			"own property is true"
		);
		t.equal(
			ES.OrdinaryHasProperty({}, "toString"),
			true,
			"inherited property is true"
		);
		t.equal(
			ES.OrdinaryHasProperty({}, "nope"),
			false,
			"absent property is false"
		);
	...
	test("InstanceofOperator", function(t) {
		...
		t.equal(
			ES.InstanceofOperator(new C(), C),
			true,
			"constructor function has an instance of itself"
		);
		t.equal(
			ES.InstanceofOperator(new D(), C),
			false,
			"constructor/instance mismatch is false"
		);
		t.equal(
			ES.InstanceofOperator(new C(), D),
			false,
			"instance/constructor mismatch is false"
		);
		t.equal(
			ES.InstanceofOperator({}, C),
			false,
			"plain object is not an instance of a constructor"
		);
		t.equal(
			ES.InstanceofOperator({}, Object),
			true,
			"plain object is an instance of Object"
		);
		...
		t.test(
			...
			function(st) {
				...
				var C2 = function() {};
				st.equal(
					ES.InstanceofOperator(O, C2),
					false,
					"O is not an instance of C2"
				);
	...
	test("Abstract Equality Comparison", function(t) {
		t.test("same types use ===", function(st) {
			forEach(v.primitives.concat(v.objects), function(value) {
				st.equal(
					ES["Abstract Equality Comparison"](value, value),
					value === value,
					debug(value) + " is abstractly equal to itself"
				);
			});
			st.end();
		});
		...
		t.test("different types coerce", function(st) {
			...
			forEach(pairs, function(pair) {
				...
				st.equal(
					ES["Abstract Equality Comparison"](a, b),
					a == b,
					debug(a) + " == " + debug(b)
				);
				// eslint-disable-next-line eqeqeq
				st.equal(
					ES["Abstract Equality Comparison"](b, a),
					b == a,
					debug(b) + " == " + debug(a)
				);
			});
	...
	test("Strict Equality Comparison", function(t) {
		t.test("same types use ===", function(st) {
			forEach(v.primitives.concat(v.objects), function(value) {
				st.equal(
					ES["Strict Equality Comparison"](value, value),
					value === value,
					debug(value) + " is strictly equal to itself"
				);
			});
			st.end();
		});
		...
		t.test("different types are not ===", function(st) {
			...
			forEach(pairs, function(pair) {
				...
				var b = pair[1];
				st.equal(
					ES["Strict Equality Comparison"](a, b),
					a === b,
					debug(a) + " === " + debug(b)
				);
				st.equal(
					ES["Strict Equality Comparison"](b, a),
					b === a,
					debug(b) + " === " + debug(a)
				);
			});
	...
	test("Abstract Relational Comparison", function(t) {
		t.test("at least one operand is NaN", function(st) {
			st.equal(
				ES["Abstract Relational Comparison"](NaN, {}, true),
				undefined,
				"LeftFirst: first is NaN, returns undefined"
			);
			st.equal(
				ES["Abstract Relational Comparison"]({}, NaN, true),
				undefined,
				"LeftFirst: second is NaN, returns undefined"
			);
			st.equal(
				ES["Abstract Relational Comparison"](NaN, {}, false),
				undefined,
				"!LeftFirst: first is NaN, returns undefined"
			);
			st.equal(
		...
		t.equal(
			ES["Abstract Relational Comparison"](3, 4, true),
			true,
			"LeftFirst: 3 is less than 4"
		);
		t.equal(
			ES["Abstract Relational Comparison"](4, 3, true),
			false,
			"LeftFirst: 3 is not less than 4"
		);
		t.equal(
			ES["Abstract Relational Comparison"](3, 4, false),
			true,
			"!LeftFirst: 3 is less than 4"
		);
		t.equal(
			ES["Abstract Relational Comparison"](4, 3, false),
			false,
			"!LeftFirst: 3 is not less than 4"
		);
		...
		t.equal(
			ES["Abstract Relational Comparison"](v.coercibleObject, 42, true),
			true,
			"LeftFirst: coercible object is less than 42"
		);
		t.equal(
			ES["Abstract Relational Comparison"](42, v.coercibleObject, true),
			false,
			"LeftFirst: 42 is not less than coercible object"
		);
		t.equal(
			ES["Abstract Relational Comparison"](v.coercibleObject, 42, false),
			true,
			"!LeftFirst: coercible object is less than 42"
		);
		t.equal(
			ES["Abstract Relational Comparison"](42, v.coercibleObject, false),
			false,
			"!LeftFirst: 42 is not less than coercible object"
		);

		t.equal(
			ES["Abstract Relational Comparison"](v.coercibleObject, "3", true),
			false,
			'LeftFirst: coercible object is not less than "3"'
		);
		t.equal(
			ES["Abstract Relational Comparison"]("3", v.coercibleObject, true),
			false,
			'LeftFirst: "3" is not less than coercible object'
		);
		t.equal(
			ES["Abstract Relational Comparison"](v.coercibleObject, "3", false),
			false,
			'!LeftFirst: coercible object is not less than "3"'
		);
		t.equal(
			ES["Abstract Relational Comparison"]("3", v.coercibleObject, false),
			false,
			'!LeftFirst: "3" is not less than coercible object'
		);
	...
	test("ValidateAndApplyPropertyDescriptor", function(t) {
		...
		forEach(v.nonUndefinedPrimitives, function(primitive) {
			...
			t["throws"](
				...
				"current: " +
					debug(primitive) +
					" is not a Property Descriptor or undefined"
			);
		});
		...
		t.test("current is undefined", function(st) {
			...
			st.test("generic descriptor", function(s2t) {
				...
				s2t.deepEqual(
					O,
					{},
					"no changes applied when O is undefined or extensible is false"
				);
				s2t.equal(
				...
				s2t.deepEqual(
					O,
					expected,
					"generic descriptor has been defined as an own data property"
				);
				s2t.end();
			});
			...
			st.test("data descriptor", function(s2t) {
				...
				s2t.deepEqual(
					O,
					expected,
					"data descriptor has been defined as an own data property"
				);
				s2t.end();
			});
			...
			st.test("accessor descriptor", function(s2t) {
				...
				s2t.deepEqual(
					O,
					expected,
					"accessor descriptor has been defined as an own accessor property"
				);
				s2t.end();
			});
		...
		t.test("every field in Desc is absent", {
			skip:
				"it is unclear if having no fields qualifies Desc to be a Property Descriptor"
		});

		forEach(
			[v.dataDescriptor, v.accessorDescriptor, v.mutatorDescriptor],
		...
		t.test(
			"Desc and current: one is a data descriptor, one is not",
			...
			function(st) {
				...
				var shouldBeAccessor = Object.getOwnPropertyDescriptor(
					startsWithData,
					"property key"
				);
				...
				st.equal(
					ES.ValidateAndApplyPropertyDescriptor(
						...
						v.descriptors.enumerable(
							v.descriptors.configurable(v.accessorDescriptor(42))
						)
					),
					true,
					"operation is successful: current is accessor, Desc is data"
				);
				var shouldBeData = Object.getOwnPropertyDescriptor(
					startsWithAccessor,
					"property key"
				);
				st.deepEqual(
					shouldBeData,
					{ configurable: true, enumerable: true, value: 42, writable: false },
					"is a data property"
				);
		...
		t.test(
			"current is nonconfigurable; Desc and current are both accessor descriptors",
			function(st) {
				st.equal(
					ES.ValidateAndApplyPropertyDescriptor(
						undefined,
	...
	test("OrdinaryGetOwnProperty", function(t) {
		...
		t.equal(
			ES.OrdinaryGetOwnProperty({}, "not in the object"),
			undefined,
			"missing property yields undefined"
		);
		t.equal(
			ES.OrdinaryGetOwnProperty({}, "toString"),
			undefined,
			"inherited non-own property yields undefined"
		);
	...
	test("OrdinaryDefineOwnProperty", { skip: !Object.defineProperty }, function(
		t
	) {
		forEach(v.primitives, function(primitive) {
			t["throws"](
	...
	test("ArrayCreate", function(t) {
		...
		t.deepEqual(
			ES.ArrayCreate(1),
			[,],
			"length of 1 creates a sparse array of length 1"
		);
		// eslint-disable-next-line no-sparse-arrays, comma-spacing
		t.deepEqual(
			ES.ArrayCreate(2),
			[, ,],
			"length of 2 creates a sparse array of length 2"
		);

		// eslint-disable-next-line no-proto
		t.test(
			"proto argument",
			{ skip: [].__proto__ !== Array.prototype },
			function(st) {
				var fakeProto = {
					push: {
						toString: function() {
				...
				st.equal(
					ES.ArrayCreate(0, fakeProto).push,
					fakeProto.push,
					"passing the proto argument works"
				);
				st.end();
			}
		);
	...
	test("GetOwnPropertyKeys", function(t) {
		...
		t.test("non-enumerable names", { skip: !Object.defineProperty }, function(
			st
		) {
			var O = { a: 1 };
	...
	test("SymbolDescriptiveString", function(t) {
		...
		t.test("Symbols", { skip: !v.hasSymbols }, function(st) {
			st.equal(
				ES.SymbolDescriptiveString(Symbol()),
				"Symbol()",
			...
			st.equal(
				ES.SymbolDescriptiveString(Symbol.iterator),
				"Symbol(Symbol.iterator)",
				"well-known symbol"
			);
			st.equal(
				ES.SymbolDescriptiveString(Symbol("foo")),
				"Symbol(foo)",
				"string description"
			);
	...
	test("GetSubstitution", { skip: skips && skips.GetSubstitution }, function(
		t
	) {
		forEach(v.nonStrings, function(nonString) {
			t["throws"](
		...
		forEach(v.nonIntegerNumbers.concat([-1, -42, -Infinity]), function(
			nonNonNegativeInteger
		) {
			t["throws"](
				function() {
		...
		for (var i = 0; i < 100; i += 1) {
			...
			if (i > 0) {
				...
				t.equal(
					...
					"supports $" +
						i +
						" at the end of the replacement, with a capture at that index"
				);
			}
			if (i < 10) {
				t.equal(
	...
	test("SecFromTime", function(t) {
		var now = new Date();
		t.equal(
			ES.SecFromTime(now.getTime()),
			now.getUTCSeconds(),
			"second from Date timestamp matches getUTCSeconds"
		);
		t.end();
	});
	...
	test("MinFromTime", function(t) {
		var now = new Date();
		t.equal(
			ES.MinFromTime(now.getTime()),
			now.getUTCMinutes(),
			"minute from Date timestamp matches getUTCMinutes"
		);
		t.end();
	});
	...
	test("HourFromTime", function(t) {
		var now = new Date();
		t.equal(
			ES.HourFromTime(now.getTime()),
			now.getUTCHours(),
			"hour from Date timestamp matches getUTCHours"
		);
		t.end();
	});
	...
	test("msFromTime", function(t) {
		var now = new Date();
		t.equal(
			ES.msFromTime(now.getTime()),
			now.getUTCMilliseconds(),
			"ms from Date timestamp matches getUTCMilliseconds"
		);
		t.end();
	});
	...
	test("Day", function(t) {
		...
		t.equal(
			ES.Day(later.getTime()),
			ES.Day(time) + Math.floor(add),
			"adding 2.5 days worth of ms, gives a Day delta of 2"
		);
		t.end();
	});
	...
	test("TimeWithinDay", function(t) {
		...
		t.equal(
			ES.TimeWithinDay(later.getTime()),
			ES.TimeWithinDay(time) + 0.5 * msPerDay,
	...
	test("DayFromYear", function(t) {
		t.equal(
			ES.DayFromYear(2021) - ES.DayFromYear(2020),
			366,
			"2021 is a leap year, has 366 days"
		);
		t.equal(
			ES.DayFromYear(2020) - ES.DayFromYear(2019),
			365,
			"2020 is not a leap year, has 365 days"
		);
		t.equal(
			ES.DayFromYear(2019) - ES.DayFromYear(2018),
			365,
			"2019 is not a leap year, has 365 days"
		);
		t.equal(
			ES.DayFromYear(2018) - ES.DayFromYear(2017),
			365,
			"2018 is not a leap year, has 365 days"
		);
		t.equal(
			ES.DayFromYear(2017) - ES.DayFromYear(2016),
			366,
			"2017 is a leap year, has 366 days"
		);
	...
	test("TimeFromYear", function(t) {
		for (var i = 1900; i < 2100; i += 1) {
			t.equal(
				ES.TimeFromYear(i),
				Date.UTC(i, 0, 1),
				"TimeFromYear matches a Date object’s year: " + i
			);
		}
		t.end();
	});
	...
	test("YearFromTime", function(t) {
		for (var i = 1900; i < 2100; i += 1) {
			t.equal(
				ES.YearFromTime(Date.UTC(i, 0, 1)),
				i,
				"YearFromTime matches a Date object’s year on 1/1: " + i
			);
			t.equal(
				ES.YearFromTime(Date.UTC(i, 10, 1)),
				i,
				"YearFromTime matches a Date object’s year on 10/1: " + i
			);
		}
		t.end();
	});
	...
	test("WeekDay", function(t) {
		...
		for (var i = 0; i < 7; i += 1) {
			...
			t.equal(
				weekDay,
				(today + i) % 7,
				i + " days after today (" + today + "), WeekDay is " + weekDay
			);
		}
		t.end();
	});
	...
	test("DayWithinYear", function(t) {
		...
		t.equal(
			ES.DayWithinYear(Date.UTC(2019, 11, 31)),
			364,
			"12/31 is the 365th day in a non leap year"
		);
		t.equal(
			ES.DayWithinYear(Date.UTC(2016, 11, 31)),
			365,
			"12/31 is the 366th day in a leap year"
		);
	...
	test("MonthFromTime", function(t) {
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 0, 1)),
			0,
			"non-leap: 1/1 gives January"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 0, 31)),
			0,
			"non-leap: 1/31 gives January"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 1, 1)),
			1,
			"non-leap: 2/1 gives February"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 1, 28)),
			1,
			"non-leap: 2/28 gives February"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 1, 29)),
			2,
			"non-leap: 2/29 gives March"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 2, 1)),
			2,
			"non-leap: 3/1 gives March"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 2, 31)),
			2,
			"non-leap: 3/31 gives March"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 3, 1)),
			3,
			"non-leap: 4/1 gives April"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 3, 30)),
			3,
			"non-leap: 4/30 gives April"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 4, 1)),
			4,
			"non-leap: 5/1 gives May"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 4, 31)),
			4,
			"non-leap: 5/31 gives May"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 5, 1)),
			5,
			"non-leap: 6/1 gives June"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 5, 30)),
			5,
			"non-leap: 6/30 gives June"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 6, 1)),
			6,
			"non-leap: 7/1 gives July"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 6, 31)),
			6,
			"non-leap: 7/31 gives July"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 7, 1)),
			7,
			"non-leap: 8/1 gives August"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 7, 30)),
			7,
			"non-leap: 8/30 gives August"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 8, 1)),
			8,
			"non-leap: 9/1 gives September"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 8, 30)),
			8,
			"non-leap: 9/30 gives September"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 9, 1)),
			9,
			"non-leap: 10/1 gives October"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 9, 31)),
			9,
			"non-leap: 10/31 gives October"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 10, 1)),
			10,
			"non-leap: 11/1 gives November"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 10, 30)),
			10,
			"non-leap: 11/30 gives November"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 11, 1)),
			11,
			"non-leap: 12/1 gives December"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2019, 11, 31)),
			11,
			"non-leap: 12/31 gives December"
		);

		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 0, 1)),
			0,
			"leap: 1/1 gives January"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 0, 31)),
			0,
			"leap: 1/31 gives January"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 1, 1)),
			1,
			"leap: 2/1 gives February"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 1, 28)),
			1,
			"leap: 2/28 gives February"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 1, 29)),
			1,
			"leap: 2/29 gives February"
		);
		t.equal(ES.MonthFromTime(Date.UTC(2016, 2, 1)), 2, "leap: 3/1 gives March");
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 2, 31)),
			2,
			"leap: 3/31 gives March"
		);
		t.equal(ES.MonthFromTime(Date.UTC(2016, 3, 1)), 3, "leap: 4/1 gives April");
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 3, 30)),
			3,
			"leap: 4/30 gives April"
		);
		...
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 5, 30)),
			5,
			"leap: 6/30 gives June"
		);
		t.equal(ES.MonthFromTime(Date.UTC(2016, 6, 1)), 6, "leap: 7/1 gives July");
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 6, 31)),
			6,
			"leap: 7/31 gives July"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 7, 1)),
			7,
			"leap: 8/1 gives August"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 7, 30)),
			7,
			"leap: 8/30 gives August"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 8, 1)),
			8,
			"leap: 9/1 gives September"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 8, 30)),
			8,
			"leap: 9/30 gives September"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 9, 1)),
			9,
			"leap: 10/1 gives October"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 9, 31)),
			9,
			"leap: 10/31 gives October"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 10, 1)),
			10,
			"leap: 11/1 gives November"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 10, 30)),
			10,
			"leap: 11/30 gives November"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 11, 1)),
			11,
			"leap: 12/1 gives December"
		);
		t.equal(
			ES.MonthFromTime(Date.UTC(2016, 11, 31)),
			11,
			"leap: 12/31 gives December"
		);
		t.end();
	});
	...
	test("DateFromTime", function(t) {
		...
		for (i = 1; i <= 28; i += 1) {
			t.equal(
				ES.DateFromTime(Date.UTC(2019, 1, i)),
				i,
				"2019.02." + i + " is date " + i
			);
		}
		for (i = 1; i <= 29; i += 1) {
			t.equal(
				ES.DateFromTime(Date.UTC(2016, 1, i)),
				i,
				"2016.02." + i + " is date " + i
			);
		}
		for (i = 1; i <= 30; i += 1) {
			t.equal(
				ES.DateFromTime(Date.UTC(2019, 8, i)),
				i,
				"2019.09." + i + " is date " + i
			);
		}
		for (i = 1; i <= 31; i += 1) {
			t.equal(
				ES.DateFromTime(Date.UTC(2019, 9, i)),
				i,
				"2019.10." + i + " is date " + i
			);
		}
	...
	test("MakeDate", function(t) {
		forEach(v.infinities.concat(NaN), function(nonFiniteNumber) {
			t.ok(
				is(ES.MakeDate(nonFiniteNumber, 0), NaN),
				debug(nonFiniteNumber) + " is not a finite `day`"
			);
			t.ok(
				is(ES.MakeDate(0, nonFiniteNumber), NaN),
				debug(nonFiniteNumber) + " is not a finite `time`"
			);
		});
		...
		t.equal(
			ES.MakeDate(0, 123),
			123,
			'zero day and nonzero time is a date of the "time"'
		);
		t.equal(
			ES.MakeDate(1, 0),
			msPerDay,
			'day of 1 and zero time is a date of "ms per day"'
		);
		t.equal(
			ES.MakeDate(3, 0),
			3 * msPerDay,
			'day of 3 and zero time is a date of thrice "ms per day"'
		);
		t.equal(
			ES.MakeDate(1, 123),
			msPerDay + 123,
			'day of 1 and nonzero time is a date of "ms per day" plus the "time"'
		);
		t.equal(
			ES.MakeDate(3, 123),
			3 * msPerDay + 123,
			'day of 3 and nonzero time is a date of thrice "ms per day" plus the "time"'
		);
	...
	test("MakeTime", function(t) {
		forEach(v.infinities.concat(NaN), function(nonFiniteNumber) {
			t.ok(
				is(ES.MakeTime(nonFiniteNumber, 0, 0, 0), NaN),
				debug(nonFiniteNumber) + " is not a finite `hour`"
			);
			t.ok(
				is(ES.MakeTime(0, nonFiniteNumber, 0, 0), NaN),
				debug(nonFiniteNumber) + " is not a finite `min`"
			);
			t.ok(
				is(ES.MakeTime(0, 0, nonFiniteNumber, 0), NaN),
				debug(nonFiniteNumber) + " is not a finite `sec`"
			);
			t.ok(
				is(ES.MakeTime(0, 0, 0, nonFiniteNumber), NaN),
				debug(nonFiniteNumber) + " is not a finite `ms`"
			);
		});
	...
	test("TimeClip", function(t) {
		forEach(v.infinities.concat(NaN), function(nonFiniteNumber) {
			t.ok(
				is(ES.TimeClip(nonFiniteNumber), NaN),
				debug(nonFiniteNumber) + " is not a finite `time`"
			);
		});
		t.ok(
			is(ES.TimeClip(8.64e15 + 1), NaN),
		...
		forEach(v.zeroes.concat([-10, 10, Date.now()]), function(time) {
			t.equal(
				ES.TimeClip(time),
				time,
				debug(time) + " is a time of " + debug(time)
			);
		});
	...
	test("ToDateString", function(t) {
		...
		t.equal(
			ES.ToDateString(now),
			Date(now),
			"any timestamp becomes `Date(timestamp)`"
		);
		t.end();
	});
	...
	test("GetPrototypeFromConstructor", function(t) {
		...
		forEach([true, "foo", 42], function(truthyPrimitive) {
			...
			t.equal(
				ES.GetPrototypeFromConstructor(f, "%Array.prototype%"),
				Array.prototype,
				"function with non-object `prototype` property (" +
					debug(truthyPrimitive) +
					") returns default intrinsic"
			);
		});
	...
	test("SetFunctionName", function(t) {
			...
			function(st) {
				...
				forEach(pairs, function(pair) {
					...
					st.equal(
						f.name,
						desc,
						debug(sym) + " yields a name of " + debug(desc)
					);
				});
		...
		var f = getNamelessFunction();
		t.test("when names are configurable", { skip: has(f, "name") }, function(
			st
		) {
			// without prefix
...
var es2016 = function ES2016(ES, ops, expectedMissing, skips) {
	...
	test(
		...
		function(t) {
			...
			t.deepEqual(
				ES.IterableToArrayLike("abc"),
				["a", "b", "c"],
				"a string of code units spreads"
			);
			t.deepEqual(
				ES.IterableToArrayLike("💩"),
				["💩"],
				"a string of code points spreads"
			);
			t.deepEqual(
				ES.IterableToArrayLike("a💩c"),
				["a", "💩", "c"],
				"a string of code points and units spreads"
			);

			var arr = [1, 2, 3];
			t.deepEqual(
				ES.IterableToArrayLike(arr),
				arr,
				"an array becomes a similar array"
			);
			t.notEqual(
				ES.IterableToArrayLike(arr),
				arr,
				"an array becomes a different, but similar, array"
			);
			...
			t.equal(
				ES.IterableToArrayLike(O),
				O,
				"a non-iterable non-array non-string object is returned directly"
			);
	...
	test("OrdinaryGetPrototypeOf", function(t) {
		...
		t.equal(
			ES.OrdinaryGetPrototypeOf({}),
			Object.prototype,
			"object [[Prototype]] is Object.prototype"
		);
		t.equal(
			ES.OrdinaryGetPrototypeOf(/a/g),
			RegExp.prototype,
			"regex [[Prototype]] is RegExp.prototype"
		);
		t.equal(
			ES.OrdinaryGetPrototypeOf(Object("")),
			String.prototype,
			"boxed string [[Prototype]] is String.prototype"
		);
		t.equal(
			ES.OrdinaryGetPrototypeOf(Object(42)),
			Number.prototype,
			"boxed number [[Prototype]] is Number.prototype"
		);
		t.equal(
			ES.OrdinaryGetPrototypeOf(Object(true)),
			Boolean.prototype,
			"boxed boolean [[Prototype]] is Boolean.prototype"
		);
		if (v.hasSymbols) {
			t.equal(
				ES.OrdinaryGetPrototypeOf(Object(Symbol.iterator)),
				Symbol.prototype,
				"boxed symbol [[Prototype]] is Symbol.prototype"
			);
		}
	...
	test("OrdinarySetPrototypeOf", function(t) {
		...
		t.equal(
			ES.OrdinarySetPrototypeOf(a, proto),
			true,
			"setting prototype is successful"
		);
...
var es2017 = function ES2017(ES, ops, expectedMissing, skips) {
	...
	test("IterableToList", function(t) {
		...
		t.test("Symbol support", { skip: !v.hasSymbols }, function(st) {
			st.deepEqual(
				ES.IterableToList("abc", String.prototype[Symbol.iterator]),
				["a", "b", "c"],
				"a string of code units spreads"
			);
			st.deepEqual(
				ES.IterableToList("☃", String.prototype[Symbol.iterator]),
				["☃"],
				"a string of code points spreads"
			);
			...
			var arr = [1, 2, 3];
			st.deepEqual(
				ES.IterableToList(arr, arr[Symbol.iterator]),
				arr,
				"an array becomes a similar array"
			);
			st.notEqual(
...
var es2018 = function ES2018(ES, ops, expectedMissing, skips) {
	...
	test("thisSymbolValue", function(t) {
		...
		t.test("symbol values", { skip: !v.hasSymbols }, function(st) {
			forEach(v.symbols, function(symbol) {
				st.equal(
					ES.thisSymbolValue(symbol),
					symbol,
					"Symbol value of " + debug(symbol) + " is same symbol"
				);
	...
	test("IsStringPrefix", function(t) {
		...
		forEach(v.strings, function(string) {
			t.equal(
				ES.IsStringPrefix(string, string),
				true,
				debug(string) + " is a prefix of itself"
			);

			t.equal(
				ES.IsStringPrefix("", string),
				true,
		...
		t.equal(
			ES.IsStringPrefix("abc", "abcd"),
			true,
			'"abc" is a prefix of "abcd"'
		);
		t.equal(
			ES.IsStringPrefix("abcd", "abc"),
			false,
			'"abcd" is not a prefix of "abc"'
		);
	...
	test("NumberToString", function(t) {
		...
		forEach(v.numbers, function(number) {
			t.equal(
				ES.NumberToString(number),
				String(number),
				debug(number) + " stringifies to " + number
			);
		});
	...
	test("CopyDataProperties", function(t) {
		...
		t.test("second argument: source", function(st) {
			...
			forEach(v.nonNullPrimitives, function(objectCoercible) {
				...
				st.deepEqual(
					keys(result),
					keys(Object(objectCoercible)),
					"target ends up with keys of " + debug(objectCoercible)
				);
			});
		...
		t.test("third argument: excludedItems", function(st) {
			...
			st.deepEqual(
				keys(result),
				["a", "c"],
				"excluded string keys are excluded"
			);
	...
	test("IsPromise", { skip: typeof Promise !== "function" }, function(t) {
		forEach(v.objects.concat(v.primitives), function(nonPromise) {
			t.equal(
				ES.IsPromise(nonPromise),
				false,
				debug(nonPromise) + " is not a Promise"
			);
		});
...
var es2019 = function ES2018(ES, ops, expectedMissing, skips) {
	...
	test("AddEntriesFromIterable", function(t) {
		...
		t.test("Symbol support", { skip: !v.hasSymbols }, function(st) {
			...
			var O = {};
			st.equal(
				ES.AddEntriesFromIterable(O, [], function() {}),
				O,
				"returns the target"
			);
	...
	test("FlattenIntoArray", function(t) {
		...
		t.test("mapper function", function(st) {
			...
			testMapper(st, double, [1, 4, [3], 8, [[[5]]]]);
			testMapper(
				st,
				function(x) {
	...
	test("TrimString", function(t) {
		...
		t.equal(
			ES.TrimString(string, "end"),
			string.slice(0, string.lastIndexOf("c") + 1)
		);
		t.equal(
			ES.TrimString(string, "start+end"),
