...
var ES6 = assign(assign({}, ES5), {
	...
	DefinePropertyOrThrow: function DefinePropertyOrThrow(O, P, desc) {
		...
		var Desc = isPropertyDescriptor(this, desc)
			? desc
			: this.ToPropertyDescriptor(desc);
		...
		return (
			...
			every(theKeys, function(k) {
				...
				if (typeof currentDesc !== "undefined") {
					...
					if (
						level === "frozen" &&
						ES.IsDataDescriptor(ES.ToPropertyDescriptor(currentDesc)) &&
	...
	InstanceofOperator: function InstanceofOperator(O, C) {
		...
		var instOfHandler =
			hasSymbols && $Symbol.hasInstance
				? this.GetMethod(C, $Symbol.hasInstance)
				: void 0;
	...
	ValidateAndApplyPropertyDescriptor: function ValidateAndApplyPropertyDescriptor(
		O,
		P,
		extensible,
		Desc,
		...
		if (oType !== "Undefined" && oType !== "Object") {
			throw new $TypeError(
				"Assertion failed: O must be undefined or an Object"
			);
		}
		...
		if (
			this.Type(current) !== "Undefined" &&
			!isPropertyDescriptor(this, current)
		) {
			throw new $TypeError(
		...
		if (this.IsGenericDescriptor(Desc)) {
			...
			if (!current["[[Configurable]]"] && !current["[[Writable]]"]) {
				...
				if (
					"[[Value]]" in Desc &&
					!this.SameValue(Desc["[[Value]]"], current["[[Value]]"])
				) {
					return false;
				}
				return true;
			...
			this.IsAccessorDescriptor(current) &&
			this.IsAccessorDescriptor(Desc)
		) {
			if (!current["[[Configurable]]"]) {
				if (
					"[[Set]]" in Desc &&
					!this.SameValue(Desc["[[Set]]"], current["[[Set]]"])
				) {
					return false;
				}
				if (
					"[[Get]]" in Desc &&
					!this.SameValue(Desc["[[Get]]"], current["[[Get]]"])
				) {
					return false;
				}
				return true;
	...
	OrdinaryDefineOwnProperty: function OrdinaryDefineOwnProperty(O, P, Desc) {
		...
		return this.ValidateAndApplyPropertyDescriptor(
			O,
			P,
			extensible,
			Desc,
	...
	GetSubstitution: function GetSubstitution(
		matched,
		str,
		position,
		captures,
			...
			if (current === "$" && !isLast) {
				...
				if (next === "$") {
					...
					if (
						...
						// if (n > m, impl-defined)
						result +=
							n <= m && this.Type(captures[n - 1]) === "Undefined"
								? ""
								: captures[n - 1];
						i += 1;
						...
						result +=
							nn <= m && this.Type(captures[nnI]) === "Undefined"
								? ""
								: captures[nnI];
						i += 2;
					} else {
	...
	GetPrototypeFromConstructor: function GetPrototypeFromConstructor(
		constructor,
		...
		if (this.Type(proto) !== "Object") {
			if (!(constructor instanceof $Function)) {
				...
				throw new $TypeError(
					"cross-realm constructors not currently supported"
				);
			}
			proto = intrinsic;
		}
